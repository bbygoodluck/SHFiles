...
void DlgEnv::OnInitDialog(wxInitDialogEvent& event)
{
	...
	wxString strFixedWidth = _gFont->IsFixedWidth() ? wxT(" (Fixed Width)") : wxT(" (Variable Width)");
...
void DlgEnv::DoApply()
{
	...
	wxString strBeforeFontName = _gFont->GetFaceName();
	if (strBeforeFontName.Cmp(strFontName) != 0)
	{
		...
		_gFont = new wxFont(iFontSize, wxFONTFAMILY_MODERN, wxFONTSTYLE_NORMAL, wxFONTWEIGHT_NORMAL, false, strFontName);
	}
