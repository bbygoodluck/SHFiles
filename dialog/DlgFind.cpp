...
//	m_bxSizerSource->Add(m_staticMSG, 0, wxALL, 5);
	...
	this->Connect(wxEVT_KEY_DOWN, wxKeyEventHandler(DlgFind::OnKeyDown));
	...
	m_txtSearchName->Connect(wxEVT_CHAR_HOOK, wxKeyEventHandler(DlgFind::OnTextCharHook), NULL, this);
}

DlgFind::~DlgFind()
{
	this->Disconnect(wxEVT_INIT_DIALOG, wxInitDialogEventHandler(DlgFind::OnInitDialog));
	this->Disconnect(wxEVT_KEY_DOWN, wxKeyEventHandler(DlgFind::OnKeyDown));
	...
	m_txtSearchName->Disconnect(wxEVT_CHAR_HOOK, wxKeyEventHandler(DlgFind::OnTextCharHook), NULL, this);
}
...
void DlgFind::OnTextCharHook(wxKeyEvent& event)
{
	ProcessKeyEvent(event.GetKeyCode());
	event.Skip();
}
...
//	int iSelPos = event.GetInt();
	...
	if(wxFileExists(strSelItem))
		strSelItem = theCommonUtil->GetParentPath(strSelItem);
	
	//선택된 아이템으로 이동
	theSplitterManager->GetActiveTab()->GetActiveViewPanel()->ReloadPathOfView(strSelItem);
}
